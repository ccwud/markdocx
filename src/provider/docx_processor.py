    ...
    def add_table(self, table_root):
        ...
        for col in table_root.thead.tr.contents:
            if col.string == "\n":
                continue
            # 处理表头内容
            header_text = col.string if col.string is not None else ""
            head_row_cells[i].paragraphs[0].add_run(header_text).bold = True
            i += 1
        ...
        for tr in table_root.tbody:
            ...
            for td in tr.contents:
                ...
                # 如果单元格包含其他元素（如链接、图片等）
                if hasattr(td, 'contents') and len(td.contents) > 0:
                    for elem in td.contents:
                        if elem.name == "a":
                            # 处理超链接
                            link_text = elem.get_text()  # 使用 get_text() 获取链接文本
                            href = elem.get("href", "")
                            if link_text:  # 只有当有文本内容时才添加链接
                                self.add_link(p, link_text, href)
                        elif elem.name == "img":
                            self.add_picture(elem)
                        elif elem.name is not None:
                            # 处理其他格式化文本
                            text = elem.get_text()  # 使用 get_text() 获取文本内容
                            if text:
                                self.add_run(p, text, elem.name)
                        elif elem.string and elem.string != "\n":
                            # 处理普通文本
                            self.add_run(p, elem.string)
                else:
                    # 处理纯文本单元格
                    cell_text = td.get_text() if hasattr(td, 'get_text') else (td.string if td.string is not None else "")
                    if cell_text:
                        p.add_run(cell_text)
                i += 1
